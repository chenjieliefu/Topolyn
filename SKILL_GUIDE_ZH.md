...
Topolyn 是适用于 Raven、Cursor、Claude Code、Codex CLI 和 OpenCode 的 Agent Skill。给它系统描述或代码仓库，就能得到可交互、可分享的专业技术地图。
...
> **当前开发版本：** `v2.16.0-dev.0`。五类图表、结构化校验、交互式查看、导出与示例画廊已经实现；稳定版发布验收仍在进行。详见[版本历史](CHANGELOG.md#unreleased)。
...
然后告诉 Agent：`使用 Topolyn 梳理这个仓库的运行时架构。`

## ❤️ 赞助伙伴

<table>
  <tr><td align="center" width="240"><a href="https://apinebula.ai/ref/wywnaATT"><img src="docs/assets/sponsors/apinebula-topolyn.jpg" alt="APINEBULA" width="200" /></a><br/><strong><a href="https://apinebula.ai/ref/wywnaATT">APINEBULA</a></strong></td><td>感谢 APINEBULA 赞助 Topolyn。APINEBULA 通过统一接口提供 Claude、GPT、Gemini 等主流模型。通过 <a href="https://apinebula.ai/ref/wywnaATT">Topolyn 专属链接</a>注册并填写优惠码 <strong><code>Topolyn</code></strong>，即可享受<strong>9 折优惠</strong>。</td></tr>
  <tr><td align="center" width="240"><a href="https://github.com/EverMind-AI/Raven"><img src="docs/assets/sponsors/evermind-topolyn-raven.png" alt="Topolyn × Raven" width="200" /></a><br/><strong><a href="https://github.com/EverMind-AI">EverMind</a> · <a href="https://github.com/EverMind-AI/Raven">Raven</a></strong></td><td>感谢 EverMind 赞助 Topolyn。EverMind 专注 Agent 记忆基础设施，旗下 <a href="https://github.com/EverMind-AI/Raven"><strong>Raven</strong></a> 已支持 Topolyn Skill，让 Raven 工作流可以直接生成经过验证的交互式系统地图。</td></tr>
</table>

> 想赞助 Topolyn？[欢迎通过邮件联系我们。](mailto:2801884530@qq.com)

## 看看 Topolyn 能做什么

下面都是真实生成的 Topolyn 成品，不是产品效果图。点击画面即可打开对应的可分享交互状态。

<p align="center">
  <a href="https://tt-a1i.github.io/archify/gallery.html"><img src="docs/assets/topolyn-live-proof.gif" alt="三个经过验证的 Topolyn 成品依次展示 Signal Flow、Blueprint 和 Classic 预设" width="960"/></a>
  <br/>
...
| 引导故事 | 路径探查 | 语义角色对比 |
|---|---|---|
| [![Agent 工作流正在播放一个作者章节](docs/assets/topolyn-demo-story.png)](https://tt-a1i.github.io/archify/gallery/artifacts/agent-tool-call.workflow.html?theme=dark&present=1&play=1#view=happy-path) | [![缓存未命中时从 Web App 到 Postgres 的路径](docs/assets/topolyn-demo-route.png)](https://tt-a1i.github.io/archify/gallery/artifacts/cache-miss.sequence.html?theme=dark&present=1#route=web~db) | [![生产架构中后端与数据库角色的真实关系](docs/assets/topolyn-demo-lens.png)](https://tt-a1i.github.io/archify/gallery/artifacts/production-deployment.architecture.html?theme=dark&present=1#lens=backend~database) |
...
| 深色 | 浅色 |
|---|---|
| ![深色主题](docs/assets/topolyn-dark.png) | ![浅色主题](docs/assets/topolyn-light.png) |

Export 菜单支持复制 PNG，并下载静态或动态格式：

![导出菜单](docs/assets/topolyn-menu.png)
...
![Route Share Card：突出 Users 到 API Server 的精确路径，同时保留完整架构作为上下文](docs/assets/topolyn-route-share-card.png)
...
[Agent 切换器](https://tt-a1i.github.io/archify/start.html?agent=cursor&type=architecture)只为 `cursor`、`codex`、`claude-code` 和 `opencode` 生成命令。Raven 仅支持 ZIP 手动安装：将 [`topolyn.zip`](topolyn.zip) 解压到 `~/.raven/workspace/skills`，解压后会得到 `~/.raven/workspace/skills/topolyn`；Raven 不属于切换器目标。
...
```text
使用 Topolyn 画出这条登录流程：Browser -> Web App -> API -> JWT 校验 ->
Redis Session 查询 -> PostgreSQL 回源。把缓存未命中作为次要路径。
```

### 3. 在对话中细调

继续说：`增加 Redis`、`把鉴权移到左侧`、`突出回滚路径`。Topolyn 会保留 Typed Source，只修改相关部分。
...
```bash
node topolyn/bin/topolyn.mjs guide "展示带 Redis 缓存未命中的 API 请求"
node topolyn/bin/topolyn.mjs guide "梳理 Kafka Topic、消费者组、重放和死信队列" --json
```

Workflow 用泳道保持主路径清晰：

![Workflow 示例](docs/assets/topolyn-workflow.png)

Sequence 解释一次交互随时间如何推进：

![Sequence 示例](docs/assets/topolyn-sequence.png)

Data Flow 突出数据移动和敏感边界：

![Data Flow 示例](docs/assets/topolyn-dataflow.png)

Lifecycle 区分正常进展、等待、重试和终态：

![Lifecycle 示例](docs/assets/topolyn-lifecycle.png)

Architecture 示例：[`Web App`](examples/web-app.html) · [`Topolyn Pipeline`](examples/topolyn-repo.html) · [`Grid 布局`](examples/topolyn-repo-grid.html) · [`桌面 Agent`](examples/maka-architecture.html)
...
Topolyn 不是通用绘图编辑器，也不是 Mermaid 主题；它负责把技术意图变成可交流的成品。
...
完整生成与 Viewer 契约请查看 [`topolyn/SKILL.md`](topolyn/SKILL.md)。

## 安装方式

| 使用位置 | 安装位置或方法 | 能力 |
|---|---|---|
| **Raven** | ZIP 手动安装：将 `topolyn.zip` 解压到 `~/.raven/workspace/skills`，解压后会得到 `~/.raven/workspace/skills/topolyn` | 完整 Renderer + Validation 工作流 |
...
| **Claude.ai** | Settings → Capabilities → Skills 中上传 `topolyn.zip` | 取决于沙箱是否提供 Node.js |
| **Project Knowledge** | 把 `topolyn.zip` 上传到项目 | Prompt 驱动的 Architecture Fallback |
Claude.ai 中的上传入口：

![Claude Skills 设置](docs/assets/claude-skills-settings.png)

**DeepSeek Harness：** 面向开发者预览版 `@deepseek-ai/dsh@0.1.0-rc.6` 的社区集成，不是 DeepSeek 官方产品；Node `^22.19.0 || >=24.0.0`。安装：`dsh plugin --profile web add @tt-a1i/archify-dsh@0.1.0`；调用：`Use the Topolyn skill to map this repository's runtime architecture.`；卸载：`dsh plugin --profile web remove @tt-a1i/archify-dsh`。没有遥测；shell 文件不会自动进入 Web Produced Files，请返回精确工作区路径。[详情](integrations/deepseek-harness/README.md)。

## 文档与范围

- [Schema 说明](topolyn/schemas/README.md)
- [Skill 与 Renderer 契约](topolyn/SKILL.md)
- [示例](topolyn/examples/)
...
较大的功能或行为调整请先通过 Issue 对齐价值、兼容边界和非目标，再基于最新 `main` 开发。一个 PR 尽量只解决一个问题；核心代码和回归测试先行，生成物最后统一重建。Topolyn 坚持 Agent-first，优先完善稳定的机器可读诊断和现有权威合同，避免新增容易与 CLI 漂移的重复说明。
