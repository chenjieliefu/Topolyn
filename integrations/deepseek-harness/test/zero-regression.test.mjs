...
test('the repository still has exactly one checked-in Topolyn SKILL.md and no generated DSH payload', () => {
  const skillFiles = git(['ls-files', '*SKILL.md']).split('\n').filter(Boolean);
  assert.deepEqual(skillFiles, ['topolyn/SKILL.md']);
...
test('Topolyn core does not import, detect, or branch on DeepSeek Harness', () => {
...
test('full-depth Skills CLI discovery still finds only one skill named archify', () => {
  ...
  const names = [...result.stdout.matchAll(/^\s*[-*]\s+(\S+)/gm)].map((match) => match[1])
    .filter((name) => name === 'topolyn' || /topolyn/i.test(name));
  const unique = new Set(
    [...result.stdout.matchAll(/\barchify\b/gi)].map((match) => match[0].toLowerCase()),
  );
  assert.ok(result.stdout.includes('topolyn'), result.stdout);
