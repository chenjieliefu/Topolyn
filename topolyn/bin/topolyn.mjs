...
function usage() {
  return `Usage:
  topolyn render <type> <input.json> [output.html] [--quality standard|showcase] [--repo-root path]
  topolyn compare architecture <base.json> <head.json> [output.html] [--receipt path] [--json] [--quality standard|showcase] [--repo-root path]
  topolyn deliver <type> <input.json> [output.html] [--json] [--open] [--quality standard|showcase] [--repo-root path]
  topolyn preview <type> <input.json> [output.html] [--no-open] [--quality standard|showcase] [--repo-root path]
  topolyn validate <type> <input.json> [--json] [--layout-json] [--quality standard|showcase] [--repo-root path]
  topolyn inspect <type> <input.json>
  topolyn check <output.html>
  topolyn visual-check <output.html> [--json]
  topolyn guide [scenario or question] [--json] [--lang en|zh]
  topolyn brands [name, alias, domain, or category] [--json]
  topolyn brands capture <url> [--json]
  topolyn examples
  topolyn doctor
  topolyn demo [output-directory]
...
async function commandCompare(args) {
  ...
  try {
    ...
    reportCompareFailure({ json: options.json, stage: 'prepare', error: 'Architecture compare runtime is unavailable.', code: 'delta/runtime-missing', details: { reason: error.message, supportedFixes: ['install the complete Topolyn skill package'] } });
    return;
  }
  const {
...
async function commandDoctor() {
  ...
  console.error(`\nTopolyn is not ready: ${problems.join('; ')}.`);
  process.exitCode = 1;
}
...
async function commandBrands(args) {
  ...
  if (positional[0] === 'capture') {
    if (positional.length !== 2) fail('Usage: topolyn brands capture <url> [--json]');
  ...
  if (json) {
    console.log(JSON.stringify({
      ...
      count: marks.length,
      marks,
      fallback: 'Run "topolyn brands capture <url> --json", then use the returned digest-pinned brand value.',
    }, null, 2));
    return;
  }
  if (!marks.length) {
    console.log(`No built-in brand matched "${query}". Run "topolyn brands capture <url> --json", then use the returned digest-pinned brand value.`);
    return;
  }
...
function commandDemo(args) {
  ...
  const output = path.join(outputDirectory, 'topolyn-demo.html');
  ...
  console.log('  topolyn render architecture <input.json> <output.html>');
}
