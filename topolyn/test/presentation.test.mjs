...
test('all typed renderers ship the same presentation stage contract', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /Topolyn\.presentation = \(function \(\)/, mode);
...
test('presentation keyboard behavior exits in layers and remains accessible', () => {
  ...
  assert.match(html, /Topolyn\.presentation\.toggle\(\)/);
  assert.match(html, /e\.key === 'Escape' && Topolyn\.focus\.active\(\)/);
  assert.match(html, /e\.key === 'Escape' && Topolyn\.presentation\.active\(\)/);
