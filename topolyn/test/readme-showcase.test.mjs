...
const assetPath = path.join(repoRoot, 'docs', 'assets', 'topolyn-live-proof.gif');
const receiptPath = path.join(repoRoot, 'docs', 'assets', 'topolyn-live-proof.json');
...
test('README motion proof is compact, looping, and backed by current gallery artifacts', () => {
  ...
  assert.equal(receipt.output, 'docs/assets/topolyn-live-proof.gif');
...
test('all skill guide languages keep the product hero and retain the verified animated proof', () => {
  for (const filename of ['SKILL_GUIDE_EN.md', 'SKILL_GUIDE_ZH.md']) {
    ...
    const demosIndex = Math.max(readme.indexOf('## See Topolyn in action'), readme.indexOf('## 看看 Topolyn 能做什么'));
...
test('README stays scannable without deleting the visual proof set', () => {
  const commonAssets = [
    'topolyn-readme-hero.svg',
    'topolyn-live-proof.gif',
    'topolyn-demo-story.png',
    'topolyn-demo-route.png',
    'topolyn-demo-lens.png',
    'mco-runtime-share-card.png',
    'topolyn-dark.png',
    'topolyn-light.png',
    'topolyn-menu.png',
    'topolyn-workflow.png',
    'topolyn-sequence.png',
    'topolyn-dataflow.png',
