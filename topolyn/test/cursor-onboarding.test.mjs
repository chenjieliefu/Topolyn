...
test('optional Cursor onboarding stays explicit and separate from the independent web product', () => {
  ...
  assert.doesNotMatch(landing, /Codex|Claude Code|OpenCode|安装 Skill|安装技能/);
  for (const surface of [english, chinese, start, landing]) {
    assert.doesNotMatch(surface, /skills use[^\n<]*--agent cursor/);
    assert.doesNotMatch(surface, /~\/\.cursor\/skills\/topolyn/);
...
test('the zero-dependency archive works from the canonical Cursor-visible agent path', () => {
  ...
  try {
    ...
    execFileSync('unzip', ['-q', path.join(repoRoot, 'topolyn.zip'), '-d', agentSkills]);
    const installed = path.join(agentSkills, 'topolyn');
    const cli = path.join(installed, 'bin', 'topolyn.mjs');
    const doctor = execFileSync(process.execPath, [cli, 'doctor'], { encoding: 'utf8' });
    assert.match(doctor, /Topolyn is ready\./);
