...
test('pointer and keyboard inspect while touch and native activation still commit directly', () => {
  ...
  assert.doesNotMatch(previewRuntime, /Topolyn\.view\.|updateUrl\(|Topolyn\.focus\.|renderStoryTrail\(/);
});
...
test('latest intent, stronger owners, playback, and lifecycle cleanup remain bounded', () => {
  ...
  assert.match(html, /Topolyn\.motionGovernor\.claim\('chapter-preview'/);
  assert.match(html, /Topolyn\.motionGovernor\.release\(token\)/);
