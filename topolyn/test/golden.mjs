...
for (const [mode, input, golden] of GOLDEN) {
  ...
  try {
    ...
    check(`${mode}: packaged ${golden}`, normalizeNewlines(fresh) === normalizeNewlines(packaged),
      `fresh render differs from topolyn/examples/${golden}; re-render the packaged examples and rebuild topolyn.zip`);
  } catch (err) {
...
check('template generator meta matches package.json version',
  template.includes(`<meta name="generator" content="topolyn ${pkg.version}">`),
...
for (const readmeName of ['README.md', 'README.zh-CN.md', 'SKILL_GUIDE_EN.md', 'SKILL_GUIDE_ZH.md']) {
