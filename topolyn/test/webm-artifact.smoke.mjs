...
try {
  ...
  async function verifyResolvedLegendContract(outputs) {
    async function inspectKinds(file, expectedKinds, theme) {
      ...
      await navigateReady(url, '!!(window.Topolyn && Topolyn.semanticLens)', `legend ${theme}`);
    ...
    await navigateReady(outputs.dataflow, '!!(window.Topolyn && Topolyn.semanticLens && Topolyn.exportMenu)', 'Dataflow database legend runtime');
    ...
    await navigateReady(outputs.custom, '!!(window.Topolyn && Topolyn.semanticLens && Topolyn.exportMenu)', 'custom legend runtime');
    const runtime = await evaluate(cdp, sessionId, String.raw`(async function () {
      ...
      var selected = Topolyn.semanticLens.active();
      var guidedActivated = Topolyn.guidedViews.activate('main', { updateUrl: false });
      var guidedActive = Topolyn.guidedViews.active();
      var visualMatrix = [];
      for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
        if (!Topolyn.preset.apply(preset)) throw new Error('could not apply preset ' + preset);
    ...
    await navigateReady(embedUrl, '!!(window.Topolyn && Topolyn.semanticLens)', 'embedded legend');
    ...
    await navigateReady(outputs.hidden, '!!(window.Topolyn && Topolyn.semanticLens)', 'hidden legend');
  ...
  async function verifySemanticPassportDismissal(file) {
    await navigateReady(file, '!!(window.Topolyn && Topolyn.focus && document.querySelector("#btn-focus-clear"))', 'Semantic Passport dismissal');
    const result = await evaluate(cdp, sessionId, `(() => {
      ...
      function state() {
        return { hidden: chip.hidden, active: Topolyn.focus.active() };
      }

      Topolyn.focus.set('clients', { toggle: false, updateUrl: false });
  ...
  async function verifyArchitectureDeltaNavigator(file) {
    ...
    const exportProof = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      ...
      var explorers = frames.map(function (frame) {
        ...
        return Boolean(child && child.Topolyn && child.Topolyn.focus && child.Topolyn.routeProbe && child.document.querySelector('#btn-node-finder') && child.document.querySelector('#guided-view-play'));
      });
      var svgA = Topolyn.deltaExport.canonicalSvg();
      document.querySelector('#theme').click();
      document.querySelector('#preset').click();
      document.querySelector('.change-row').click();
      var svgB = Topolyn.deltaExport.canonicalSvg();
      var parsed = new DOMParser().parseFromString(svgB, 'image/svg+xml');
      var exportStyle = parsed.querySelector('style')?.textContent || '';
      var blob = await Topolyn.deltaExport.shareCard();
  ...
  async function captureShareCard(file, label) {
    await navigateReady(file, '!!(window.Topolyn && Topolyn.exportMenu && Topolyn.exportMenu.shareCard)', label);
    const sharePayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        var blob = await Topolyn.exportMenu.shareCard();
        var bytes = new Uint8Array(await blob.arrayBuffer());
        var binary = '';
  ...
  async function captureCopiedShareCard(file, label) {
    await navigateReady(file, '!!(window.Topolyn && Topolyn.exportMenu && Topolyn.exportMenu.copyShareCard)', label);
    const copiedPayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        ...
        await Topolyn.exportMenu.copyShareCard();
  ...
  async function captureRouteShareCard(file, label, sourceId, targetId, options = {}) {
    await navigateReady(file, '!!(window.Topolyn && Topolyn.routeProbe && Topolyn.exportMenu && Topolyn.exportMenu.downloadRouteShareCard)', label);
    const routePayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        ...
        Topolyn.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
        if (!Topolyn.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) {
          throw new Error('route did not resolve');
        }
        var snapshot = Topolyn.routeProbe.exportSnapshot();
        if (!snapshot) throw new Error('resolved route exposed no export snapshot');
        Topolyn.exportMenu.syncRouteShare();
        ...
        var duplicateGeometryRejected = Topolyn.routeProbe.exportSnapshot() === null;
        var duplicateExportError = '';
        try { await Topolyn.exportMenu.shareCard({ variant: 'route' }); }
        ...
        var emptyGeometryRejected = Topolyn.routeProbe.exportSnapshot() === null;
        ...
        try {
          blob = await Topolyn.exportMenu.downloadRouteShareCard();
          routeReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
          ...
          if (${JSON.stringify(options.journeyInvariance === true)}) {
            async function captureJourneySource(action) {
              ...
              await Topolyn.exportMenu.shareCard({ variant: 'route' });
              routeFingerprints.push(fingerprint(await captured[captureIndex]));
            }
            await captureJourneySource(function () { Topolyn.routeProbe.selectJourneyIndex(0); });
            await captureJourneySource(function () { Topolyn.routeProbe.selectJourneyIndex(Math.floor(snapshot.nodeIds.length / 2)); });
            await captureJourneySource(function () { Topolyn.routeProbe.selectJourneyIndex(snapshot.nodeIds.length - 1); });
            Topolyn.routeProbe.showOverview({ reveal: false });
            var started = Topolyn.routeProbe.playJourney();
            ...
            Topolyn.routeProbe.pauseJourney({ preserveElapsed: false });
            await captureJourneySource(function () {});
          ...
          await Topolyn.exportMenu.run('png');
          var pngReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            canonical: document.documentElement.getAttribute('data-last-export-canonical')
          ...
          var asyncClearPromise = Topolyn.exportMenu.shareCard({ variant: 'route' });
          Topolyn.routeProbe.clear({ updateUrl: false, preserveView: true });
          ...
          await Topolyn.exportMenu.downloadRouteShareCard();
          var failedReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
          ...
          return {
            ...
            staleSnapshot: Topolyn.routeProbe.exportSnapshot(),
  ...
  async function verifyDynamicReducedMotionRoute(file, label, sourceId, targetId) {
    ...
    await navigateReady(file, '!!(window.Topolyn && Topolyn.routeProbe && Topolyn.exportMenu && Topolyn.motionGovernor)', label);
    ...
    const setup = await evaluate(cdp, sessionId, `(function () {
      Topolyn.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
      if (!Topolyn.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) return { resolved: false };
      Topolyn.routeProbe.showOverview({ reveal: false });
      return {
        resolved: true,
        started: Topolyn.routeProbe.playJourney(),
        playing: Topolyn.routeProbe.isJourneyPlaying(),
  ...
  async function captureRouteVisualMatrix(file, label, sourceId, targetId) {
    await navigateReady(file, '!!(window.Topolyn && Topolyn.preset && Topolyn.routeProbe && Topolyn.exportMenu)', label);
    const matrix = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      Topolyn.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
      if (!Topolyn.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) {
        throw new Error('route did not resolve');
      }
      var identity = JSON.stringify(Topolyn.routeProbe.exportSnapshot());
      var results = [];
      for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
        if (!Topolyn.preset.apply(preset)) throw new Error('could not apply preset ' + preset);
        for (var theme of ['dark', 'light']) {
          document.documentElement.setAttribute('data-theme', theme);
          var blob = await Topolyn.exportMenu.shareCard({ variant: 'route' });
          var bytes = new Uint8Array(await blob.arrayBuffer());
          var hash = 2166136261;
          ...
          results.push({
            ...
            identity: JSON.stringify(Topolyn.routeProbe.exportSnapshot())
          });
        }
      }
  ...
  async function captureReachShareCard(file, label, originId, direction, options = {}) {
    await navigateReady(file, '!!(window.Topolyn && Topolyn.focus && Topolyn.focus.reachabilitySnapshot && Topolyn.exportMenu && Topolyn.exportMenu.downloadReachShareCard)', label);
    const reachPayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        ...
        if (!Topolyn.focus.set(${JSON.stringify(originId)}, { toggle: false, updateUrl: false })) {
          throw new Error('focus origin did not resolve');
        }
        if (!Topolyn.focus.reach(${JSON.stringify(direction)}, { toggle: false, updateUrl: false, reveal: false })) {
          throw new Error('authored reach did not resolve');
        }
        var snapshot = Topolyn.focus.reachabilitySnapshot();
        if (!snapshot) throw new Error('active authored reach exposed no export snapshot');
        Topolyn.exportMenu.syncReachShare();
        ...
        svg.appendChild(duplicateCarrier);
        var duplicateGeometryRejected = Topolyn.focus.reachabilitySnapshot() === null;
        var duplicateExportError = '';
        try { await Topolyn.exportMenu.shareCard({ variant: 'reach' }); }
        catch (error) { duplicateExportError = String(error && error.message || error); }
        duplicateCarrier.remove();
        snapshot = Topolyn.focus.reachabilitySnapshot();
        ...
        try {
          var blob = await Topolyn.exportMenu.shareCard({ variant: 'reach' });
          var reachSvgText = captured[0] ? await captured[0] : '';
          var downloadedBlob = await Topolyn.exportMenu.downloadReachShareCard();
          var reachReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            width: document.documentElement.getAttribute('data-last-export-width'),
          ...
          if (${JSON.stringify(options.matrix === true)}) {
            ...
            for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
              ...
              for (var theme of ['dark', 'light']) {
                document.documentElement.setAttribute('data-theme', theme);
                var matrixBlob = await Topolyn.exportMenu.shareCard({ variant: 'reach' });
                ...
                matrix.push({
                  ...
                  identity: JSON.stringify(Topolyn.focus.reachabilitySnapshot())
                });
              }
            }
          ...
          Topolyn.focus.clearReach({ updateUrl: false });
          Topolyn.exportMenu.syncReachShare();
          ...
          await Topolyn.exportMenu.downloadReachShareCard();
          var failedReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            errorFormat: document.documentElement.getAttribute('data-last-export-error-format'),
          ...
          var canonicalIndex = captured.length;
          var canonicalBlob = await Topolyn.exportMenu.shareCard();
          var canonicalSvgText = captured[canonicalIndex] ? await captured[canonicalIndex] : '';
          var canonicalSvg = parser.parseFromString(canonicalSvgText, 'image/svg+xml').documentElement;
          ...
          return {
            ...
            hiddenAfterClear: hiddenAfterClear,
            staleSnapshot: Topolyn.focus.reachabilitySnapshot(),
            staleError: staleError,
            failedReceipt: failedReceipt,
            canonicalSize: canonicalBlob.size,
  ...
  await navigateReady(output, '!!(window.Topolyn && Topolyn.motion && Topolyn.motion.canRecord())', 'motion artifact');

  const payload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
    try {
      var blob = await Topolyn.motion.recordWebm({ duration: 1400, fps: 12 });
      var bytes = new Uint8Array(await blob.arrayBuffer());
      var binary = '';
      for (var offset = 0; offset < bytes.length; offset += 32768) {
        binary += String.fromCharCode.apply(null, bytes.subarray(offset, offset + 32768));
