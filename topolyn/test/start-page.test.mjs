...
test('start page: offers five bounded bilingual starts without ingesting source content', () => {
  ...
  assert.match(html, /npx -y skills add tt-a1i\/archify --skill topolyn --agent codex --global --copy --yes/);
  assert.match(html, /npx -y skills add tt-a1i\/archify --skill topolyn --agent codex --copy --yes/);
...
test('generated artifacts omit the promotional footer and shortcut manual', () => {
  ...
  try {
    for (const [type, input] of Object.entries(examples)) {
      ...
      assert.doesNotMatch(html, /Built with Topolyn/, `${type}: product signature`);
...
test('viewer gives wide screens a larger canvas without forcing a subtitle row', () => {
  ...
  assert.match(template, /Topolyn\.readerLayout = \(function \(\)/);
