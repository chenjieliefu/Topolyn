...
function writeValidDevelopmentFixture(root, overrides = {}) {
  ...
  const english = [
    ...
    '',
    'Raven uses manual ZIP installation: extract topolyn.zip into `~/.raven/workspace/skills`, which yields `~/.raven/workspace/skills/topolyn`; Raven is not an agent-switcher target.',
  ].join('\n');
  const chinese = [
    ...
    `当前开发版本：\`v${version}\``,
    '',
    'Raven 使用 ZIP 手动安装：将 topolyn.zip 解压到 `~/.raven/workspace/skills`，解压后会得到 `~/.raven/workspace/skills/topolyn`；Raven 不属于 Agent 切换器目标。',
  ].join('\n');
  const files = {
    'topolyn/package.json': JSON.stringify({ version }),
    'topolyn/package-lock.json': JSON.stringify({ version, packages: { '': { version } } }),
    'topolyn/SKILL.md': '---\nmetadata:\n  version: "2.13"\n---\n',
    'topolyn/assets/template.html': '<meta name="generator" content="topolyn 2.13.0-dev.0">',
    'CHANGELOG.md': [
      '# Changelog',
      '',
      '## [Unreleased]',
    ...
    'docs/index.html': `<span>development · v${version} · 开发版</span><a href="workspace.html">Workspace</a><a href="guide.html">Guide</a><a href="gallery.html">Gallery</a>`,
    'docs/workspace.html': `<span>development · v${version} · 开发版</span><textarea id="system-prompt"></textarea><button id="generate-button">Generate</button><script src="assets/topolyn-app.js"></script>`,
    'docs/start.html': `<span>development · v${version} · 开发版</span><p>Raven manual ZIP / ZIP 手动安装: extract topolyn.zip into ~/.raven/workspace/skills, which yields ~/.raven/workspace/skills/topolyn; 将 topolyn.zip 解压到 ~/.raven/workspace/skills，解压后会得到 ~/.raven/workspace/skills/topolyn; not an agent-switcher target.</p>`,
...
function writeValidStableFixture(root, overrides = {}) {
  ...
  const files = {
    ...
    'topolyn/package-lock.json': JSON.stringify({ version, packages: { '': { version } } }),
    'topolyn/SKILL.md': '---\nmetadata:\n  version: "2.13"\n---\n',
    'topolyn/assets/template.html': '<meta name="generator" content="topolyn 2.13.0">',
    'CHANGELOG.md': [
      '# Changelog',
      '',
      '## [Unreleased]',
    ...
    'docs/index.html': `<span>stable · v${version} · 稳定版</span><a href="workspace.html">Workspace</a><a href="guide.html">Guide</a><a href="gallery.html">Gallery</a>`,
    'docs/workspace.html': `<span>stable · v${version} · 稳定版</span><textarea id="system-prompt"></textarea><button id="generate-button">Generate</button><script src="assets/topolyn-app.js"></script>`,
    'docs/start.html': `<span>stable · v${version} · 稳定版</span><p>Raven manual ZIP / ZIP 手动安装: extract topolyn.zip into ~/.raven/workspace/skills, which yields ~/.raven/workspace/skills/topolyn; 将 topolyn.zip 解压到 ~/.raven/workspace/skills，解压后会得到 ~/.raven/workspace/skills/topolyn; not an agent-switcher target.</p>`,
...
test('real Unreleased changes cannot reuse a stable published package identity', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'archify-release-identity-'));
  try {
    writeFile(fixture, 'topolyn/package.json', JSON.stringify({ version: '2.12.0' }));
...
test('package, lockfile, Skill metadata, escaped Shields badge, and public docs share one development identity', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'archify-release-identity-'));
  try {
    writeFile(fixture, 'topolyn/package.json', JSON.stringify({ version: '2.13.0-dev.0' }));
    writeFile(fixture, 'topolyn/package-lock.json', JSON.stringify({
      version: '2.12.0',
      packages: { '': { version: '2.12.0' } },
    }));
    writeFile(fixture, 'topolyn/SKILL.md', '---\nmetadata:\n  version: "2.12"\n---\n');
    writeFile(fixture, 'CHANGELOG.md', [
      '# Changelog',
      '',
      '## [Unreleased]',
...
test('landing must expose the complete four-page product navigation', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'archify-release-identity-'));
  try {
    writeValidDevelopmentFixture(fixture, {
      'docs/index.html': '<span>development · v2.13.0-dev.0 · 开发版</span>',
    });

    const result = runCheck(fixture);
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /docs\/index\.html must expose the complete four-page Topolyn product navigation/);
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
});

test('landing and workspace remain separate product surfaces', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'archify-release-identity-'));
  try {
    writeValidDevelopmentFixture(fixture, {
      'docs/index.html': [
...
test('Raven instructions reject extracting the archive into the final Skill directory', () => {
  ...
  try {
    const nestedEnglish = [
      ...
      '',
      'Raven is manual ZIP only: extract topolyn.zip into `~/.raven/workspace/skills/topolyn`; Raven is not an agent-switcher target.',
    ].join('\n');
    writeValidDevelopmentFixture(fixture, {
    ...
    const result = runCheck(fixture);
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /extract topolyn\.zip into ~\/\.raven\/workspace\/skills, yielding ~\/\.raven\/workspace\/skills\/topolyn/);
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
});
...
test('renderer template generator carries the complete package prerelease identity', () => {
  ...
  try {
    writeValidDevelopmentFixture(fixture, {
      'topolyn/assets/template.html': '<meta name="generator" content="topolyn 2.12.0">',
    });

    const result = runCheck(fixture);
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /topolyn\/assets\/template\.html generator must be topolyn 2\.13\.0-dev\.0/);
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
});
