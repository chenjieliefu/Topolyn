...
test('release smokes the exact archive built for the release before freshness and upload', () => {
  ...
  assert.match(tagGate, /require\('\.\/topolyn\/package\.json'\)\.version/);
  assert.match(tagGate, /GITHUB_REF_NAME#v/);
  assert.match(build, /run: scripts\/build-zip\.sh \/tmp\/topolyn-built\.zip/);
  assert.match(smoke, /unzip -q \/tmp\/topolyn-built\.zip -d "\$package_root"/);
  assert.match(smoke, /node scripts\/package-smoke\.mjs "\$package_root\/topolyn"/);
  assert.doesNotMatch(smoke, /\bnpm\s+(?:ci|install)\b/);
  assert.match(freshness, /cmp -s \/tmp\/topolyn-built\.zip topolyn\.zip/);
  assert.match(upload, /files: topolyn\.zip/);
});
...
test('package smoke rejects every dependency or repository-only artifact', () => {
  ...
  for (const { relative, kind } of forbidden) {
    const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'topolyn-package-gate-'));
    try {
      fs.mkdirSync(path.join(fixture, 'bin'), { recursive: true });
      fs.writeFileSync(path.join(fixture, 'bin', 'topolyn.mjs'), '');
...
canonicalZipTest('package smoke rejects every dependency metadata field in a built package', () => {
  ...
  try {
    const archive = path.join(fixture, 'topolyn.zip');
    ...
    const builtPackage = path.join(extracted, 'topolyn');
...
canonicalZipTest('archive build excludes untracked files and external symlinks from the live working tree', () => {
  ...
  const untracked = path.join(repoRoot, 'topolyn', `${marker}.txt`);
  const externalRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'topolyn-package-external-'));
  const externalTarget = path.join(externalRoot, 'secret.txt');
  const externalLink = path.join(repoRoot, 'topolyn', `${marker}.link`);
  const outputRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'topolyn-package-negative-'));
  const archive = path.join(outputRoot, 'topolyn.zip');
...
canonicalZipTest('archive build rejects an unmerged index and preserves an existing archive', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'topolyn-package-unmerged-'));
  const scripts = path.join(fixture, 'scripts');
  const skill = path.join(fixture, 'topolyn');
  ...
  try {
    ...
    const indexInfo = [
      `100644 ${base.stdout.trim()} 1\ttopolyn/LICENSE`,
      `100644 ${ours.stdout.trim()} 2\ttopolyn/LICENSE`,
      `100644 ${theirs.stdout.trim()} 3\ttopolyn/LICENSE`,
      '',
    ].join('\n');
...
test('archive build rejects non-canonical Node versions before publishing output', {
  ...
  const outputRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'topolyn-package-node-version-'));
  try {
    const archive = path.join(outputRoot, 'topolyn.zip');
    ...
    assert.notEqual(build.status, 0, `${build.stdout}\n${build.stderr}`);
    assert.match(build.stderr, /canonical topolyn\.zip builds require Node 22/);
...
canonicalZipTest('archive build is byte-for-byte reproducible across caller time zones without system zip', () => {
  const outputRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'topolyn-package-reproducible-'));
  ...
  try {
    ...
    assert.ok(
      fs.readFileSync(utcArchive).equals(fs.readFileSync(path.join(repoRoot, 'topolyn.zip'))),
...
test('CI tests the declared Node floor plus every maintained current lane', () => {
  const packageJson = JSON.parse(fs.readFileSync(path.join(repoRoot, 'topolyn', 'package.json'), 'utf8'));
