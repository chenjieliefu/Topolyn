...
test('all typed renderers inherit one geometry-neutral Intent Trace', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /Topolyn\.intentTrace = \(function \(\)/, mode);
...
test('Intent Trace separates hover, keyboard, touch, and committed focus', () => {
  ...
  assert.match(html, /Topolyn\.focus\.active\(\)/);
  assert.match(html, /Topolyn\.intentTrace\.clear\(\{ announce: false \}\)/);
  assert.match(html, /e\.key === 'Escape' && Topolyn\.intentTrace\.active\(\)/);
});
