...
test('motion, camera, layered Escape, mobile, print, and embed boundaries stay explicit', () => {
  assert.match(template, /Topolyn\.motionGovernor\.capable === true[\s\S]*?!Topolyn\.motionGovernor\.isPaused\(\)/);
  assert.match(template, /Topolyn\.motionGovernor\.claim\('route'/);
  assert.match(template, /reason: 'route-journey',[\s\S]*?maxScale: 1\.65,[\s\S]*?padding: 64,[\s\S]*?duration: 360/);
  assert.match(template, /Topolyn\.routeProbe\.pauseJourney\(\{ preserveElapsed: true, reason: reason \|\| 'manual' \}\)/);
  ...
  assert.match(template, /Topolyn\.routeProbe\.escape\(\{ restoreFocus: true \}\)/);
