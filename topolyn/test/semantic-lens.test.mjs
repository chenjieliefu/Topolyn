...
test('all typed renderers inherit one viewer-only Semantic Lens', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /Topolyn\.semanticLens = \(function \(\)/, mode);
...
test('Semantic Lens is shareable and yields cleanly to stronger reader intent', () => {
  ...
  assert.match(html, /Topolyn\.semanticLens\.clear\(\{ updateUrl: false/);
  assert.match(html, /Topolyn\.focus\.clear\(\{ updateUrl: false, preserveView: true \}\)/);
  assert.match(html, /Topolyn\.routeProbe\.clear\(\{ updateUrl: false, restoreFocus: false \}\)/);
  assert.match(html, /Topolyn\.guidedViews\.showAll\(\{ clearFocus: false, updateUrl: false \}\)/);
  assert.match(html, /if \(action === 'lens'\) return Topolyn\.semanticLens\.open\(\)/);
  assert.match(html, /e\.key === 'l' \|\| e\.key === 'L'/);
  assert.match(html, /e\.key === 'Escape' && Topolyn\.semanticLens\.isOpen\(\)/);
  assert.match(html, /e\.key === 'Escape' && Topolyn\.semanticLens\.active\(\)/);
});
