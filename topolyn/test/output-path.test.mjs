...
test('future-path aliases follow the containing directory case and Unicode semantics', () => {
  ...
  const caseInsensitive = directoryAliasesNames(cwd, 'TopolynCaseProbe', 'topolyncaseprobe');
...
test('deliver rechecks aliases immediately before committing a verified candidate', { timeout: 10000 }, async () => {
  ...
  fs.copyFileSync(path.join(skillRoot, 'bin', 'topolyn.mjs'), path.join(installedBin, 'topolyn.mjs'));
  fs.copyFileSync(
...
`);
  ...
  const child = spawn(process.execPath, [
    path.join(installedBin, 'topolyn.mjs'),
...
test('doctor reports a missing output-path safety runtime in an installed skill', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(installedRoot, 'bin/topolyn.mjs'), 'doctor'], {
    cwd: installedRoot,
    encoding: 'utf8',
  });
