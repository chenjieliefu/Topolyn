...
test('all typed renderers inherit one viewer-only Diagram Guide', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /Topolyn\.guide = \(function \(\)/, mode);
    assert.match(html, /Diagram Guide — a factual command deck over existing interactions/, mode);
    assert.doesNotMatch(canonicalSvg(html), /diagram-guide|Topolyn\.guide|Explore this system/, mode);
  }
});
...
test('Diagram Guide reports compiled semantic facts and honest story availability', () => {
  ...
  assert.match(html, /return Topolyn\.guidedViews && Number\(Topolyn\.guidedViews\.count\) \|\| 0/);
...
test('Diagram Guide delegates its task rows to existing production interactions', () => {
  ...
  assert.match(html, /if \(action === 'find'\) return Topolyn\.finder\.open\(\)/);
  assert.match(html, /if \(action === 'route'\) return Topolyn\.routeProbe\.begin\(\{ focusNode: true \}\)/);
  assert.match(html, /if \(action === 'map'\) return Topolyn\.radar\.open\(\)/);
  assert.match(html, /if \(action === 'story'\) return Topolyn\.guidedViews\.play\(\)/);
  assert.match(html, /if \(action === 'present'\) return Topolyn\.presentation\.enter\(\)/);
  assert.match(html, /if \(action === 'export'\) return Topolyn\.exportMenu\.open\(\)/);
  assert.match(html, /if \(action === 'theme'\) return Topolyn\.theme\.toggle\(\)/);
  assert.match(html, /if \(action === 'reset'\) return Topolyn\.view\.reset\(\)/);
  assert.match(html, /Topolyn\.guidedViews\.pause\(\)/);
  assert.match(html, /Topolyn\.finder\.close\(\{ restoreFocus: false \}\)/);
  assert.match(html, /Topolyn\.radar\.close\(\{ restoreFocus: false \}\)/);
...
test('Diagram Guide is keyboard-first, mobile-contained, motion-safe, and embed-clean', () => {
  ...
  assert.match(html, /Topolyn\.guide\.toggle\(\)/);
