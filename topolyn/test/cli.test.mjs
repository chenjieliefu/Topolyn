...
test('cli: help lists commands and diagram types', () => {
  ...
  assert.match(result.stdout, /topolyn render <type>/);
  assert.match(result.stdout, /topolyn compare architecture <base\.json> <head\.json>/);
  assert.match(result.stdout, /topolyn deliver <type>/);
  assert.match(result.stdout, /topolyn preview <type>/);
  assert.match(result.stdout, /topolyn visual-check <output\.html>/);
  assert.match(result.stdout, /--open/);
  assert.match(result.stdout, /topolyn guide \[scenario or question\]/);
  assert.match(result.stdout, /topolyn doctor/);
  assert.match(result.stdout, /topolyn demo \[output-directory\]/);
...
test('cli: doctor reports a complete installation is ready', () => {
  ...
  assert.match(result.stdout, /Topolyn is ready\./);
});
...
test('cli: doctor identifies an incomplete installation', () => {
  ...
  fs.copyFileSync(path.join(skillRoot, 'bin', 'topolyn.mjs'), path.join(incompleteBin, 'topolyn.mjs'));

  const result = spawnSync(process.execPath, [path.join(incompleteBin, 'topolyn.mjs'), 'doctor'], {
    cwd: incompleteRoot,
    encoding: 'utf8',
  });
  ...
  assert.match(result.stderr, /Topolyn is not ready: \d+ required files? missing\./);
});
...
test('cli: doctor rejects a corrupt standalone validator', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(corruptRoot, 'bin/topolyn.mjs'), 'doctor'], {
    cwd: corruptRoot,
    encoding: 'utf8',
  });
  ...
  assert.match(result.stderr, /Topolyn is not ready: 1 runtime check failed\./);
});
...
test('cli: examples renders from an installed skill', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(installedRoot, 'bin/topolyn.mjs'), 'examples'], {
    cwd: installedRoot,
    encoding: 'utf8',
  });
...
test('cli: guide lists all scenario recipes by diagram type', () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /Topolyn scenario recipes \(11\)/);
...
test('cli: guide works from an installed skill without node_modules', () => {
  ...
  copyInstalledSkill(installedRoot);
  const installedCli = path.join(installedRoot, 'bin/topolyn.mjs');
...
test('cli: demo creates a ready-to-open diagram in a chosen directory', () => {
  ...
  assert.match(result.stdout, /topolyn render architecture/);
});
...
test('cli: demo defaults to the current directory', () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.equal(fs.existsSync(path.join(workingDirectory, 'topolyn-demo.html')), true);
});
...
test('cli: inspect emits architecture layout json', () => {
  const input = path.resolve(skillRoot, '../examples/topolyn-repo-grid.architecture.json');
