...
test('Route Share Card menu lifecycle excludes hidden state from keyboard navigation', () => {
  ...
  assert.match(html, /function clear\(options\)[\s\S]*?Topolyn\.exportMenu\.syncRouteShare\(\)/);
  assert.match(html, /function showResult\(result, options\)[\s\S]*?Topolyn\.exportMenu\.syncRouteShare\(\)/);
...
test('Route Share Card reuses one 1200x630 variant seam and publishes a truthful receipt', () => {
  ...
  assert.match(html, /var snapshot = Topolyn\.routeProbe && Topolyn\.routeProbe\.exportSnapshot\(\)/);
  ...
  assert.match(html, /clearExportReceipt\(\);[\s\S]*?var snapshot = Topolyn\.routeProbe/);
...
test('skill and READMEs describe the optional Export variant and show one real card without changing the hero', () => {
  ...
  const png = fs.readFileSync(path.join(repoRoot, 'docs/assets/topolyn-route-share-card.png'));
