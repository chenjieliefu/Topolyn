...
test('all typed renderers ship the same geometry-neutral node finder', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /Topolyn\.finder = \(function \(\)/, mode);
    assert.match(html, /svg\.querySelectorAll\('\[data-node-id\]'\)/, mode);
    assert.doesNotMatch(svg(html), /node-finder|Topolyn\.finder|Find a node/, mode);
  }
});
...
test('finder searches semantic ids and labels, then delegates to focus and reveal', () => {
  ...
  assert.match(html, /Topolyn\.guidedViews\.showAll\(\{ clearFocus: false, updateUrl: false \}\)/);
  assert.match(html, /Topolyn\.view\.reset\(\{ automatic: true \}\)/);
  assert.match(html, /Topolyn\.focus\.set\(id, \{ toggle: false \}\)/);
  assert.match(html, /Topolyn\.view\.reveal\(\[id\], \{ includeNeighbors: true, reason: 'finder' \}\)/);
...
test('finder becomes a contextual Route Probe endpoint picker without changing semantic focus', () => {
  ...
  assert.match(html, /Topolyn\.routeProbe\.finderContext\(\)/);
  ...
  assert.match(html, /Topolyn\.routeProbe\.choose\(id\)/);
