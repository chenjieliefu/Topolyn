...
explicit authored viewBox without overlap, Topolyn omits the complete legend
...
by `topolyn brands --json`, or a digest-pinned `{ "url", "sha256" }` object
returned by `topolyn brands capture <url> --json`. Known IDs and known-brand
...
strictly Topolyn judges composition. `engineering_profile` is an optional
