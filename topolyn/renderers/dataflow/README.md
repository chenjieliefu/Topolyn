...
Render `diagram_type: "dataflow"` JSON files into the standard Topolyn HTML
template.

```bash
node topolyn/renderers/dataflow/render-dataflow.mjs input.dataflow.json output.html
```

The renderer validates input against `topolyn/schemas/dataflow.schema.json`
...
`topolyn/examples/product-analytics.dataflow.json`.

The schema lives at:

```text
topolyn/schemas/dataflow.schema.json
```
...
visual-only because Topolyn has no compiled edge-kind facts in this slice. A
