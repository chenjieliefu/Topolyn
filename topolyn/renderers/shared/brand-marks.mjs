...
const USER_AGENT = 'Topolyn/2.15 brand-preview';
...
export async function prepareDiagramBrandMarks(diagramType, diagram) {
  ...
  await mapConcurrent(nodes, MAX_CAPTURE_CONCURRENCY, async (node, index) => {
    ...
    if (url) {
      unknown.push(`/${collection}/${index}/brand ${JSON.stringify(node.brand)} is an unpinned URL; capture it first with \`topolyn brands capture ${url.href} --json\``);
      return;
    }
  ...
  if (unknown.length) {
    throwDiagnosticError(`Brand mark validation failed:\n- ${unknown.join('\n- ')}`, unknown.map((message) => ({
      ...
      supportedFixes: message.includes('is an unpinned URL')
        ? ['run `topolyn brands capture <url> --json` and author the returned digest-pinned brand object']
        : ['choose an ID from `topolyn brands`', 'run `topolyn brands capture <url> --json` for an unknown official site'],
    })));
  }
}
