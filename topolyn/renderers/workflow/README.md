...
Render `diagram_type: "workflow"` JSON files into the standard Topolyn HTML
template.

```bash
node topolyn/renderers/workflow/render-workflow.mjs input.workflow.json output.html
```

The renderer validates input against `topolyn/schemas/workflow.schema.json`
...
```bash
node topolyn/scripts/check-render-output.mjs output.html
```
...
`topolyn/examples/agent-tool-call.workflow.json`.

The schema lives at:

```text
topolyn/schemas/workflow.schema.json
```
