...
Render `diagram_type: "sequence"` JSON files into the standard Topolyn HTML
template.

```bash
node topolyn/renderers/sequence/render-sequence.mjs input.sequence.json output.html
```

The renderer validates input against `topolyn/schemas/sequence.schema.json`
...
`topolyn/examples/cache-miss-request.sequence.json`.

The schema lives at:

```text
topolyn/schemas/sequence.schema.json
```
