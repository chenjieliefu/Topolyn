...
Render `diagram_type: "lifecycle"` JSON files into the standard Topolyn HTML
template.

```bash
node topolyn/renderers/lifecycle/render-lifecycle.mjs input.lifecycle.json output.html
```

The renderer validates input against `topolyn/schemas/lifecycle.schema.json`
...
`topolyn/examples/agent-run.lifecycle.json`.

The schema lives at:

```text
topolyn/schemas/lifecycle.schema.json
```
