# Topolyn first-class Cursor onboarding research

> Snapshot: 2026-07-23. Sources are limited to Cursor's official documentation and changelog, the official `vercel-labs/skills` repository and CLI, Topolyn's own issue/docs/tests, and first-party peer repositories.
...
npx -y skills add tt-a1i/archify --skill topolyn --agent cursor --copy --yes
```

For a reproducible test fixture, pin the version that was verified in this snapshot:

```bash
npx -y skills@1.5.20 add tt-a1i/archify --skill topolyn --agent cursor --copy --yes
```
...
| Scope | Valid Skill roots | Topolyn entry |
|---|---|---|
| Project | `.agents/skills/`, `.cursor/skills/` | `<root>/topolyn/SKILL.md` |
| User/global | `~/.agents/skills/`, `~/.cursor/skills/` | `<root>/topolyn/SKILL.md` |
...
- The current Topolyn README names Claude, Codex CLI, and opencode, and its installation matrix does not include Cursor.[Current positioning](https://github.com/tt-a1i/archify/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/README.md#L11-L19) · [Current installation matrix](https://github.com/tt-a1i/archify/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/README.md#L223-L231)
- Topolyn requires Node.js 18 or newer, but its distributed renderer and validators do not require `npm install`.[Runtime contract](https://github.com/tt-a1i/archify/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/archify/package.json#L8-L13) · [Skill setup contract](https://github.com/tt-a1i/archify/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/archify/SKILL.md#L27-L33)
...
npx -y skills@1.5.20 add tt-a1i/archify \
  --skill topolyn --agent cursor --copy --yes
```

Observed evidence:

1. `npx -y skills@1.5.20 list --agent cursor --json` reported one project-scoped `Cursor` installation at `.agents/skills/topolyn`.
2. `node .agents/skills/topolyn/bin/topolyn.mjs doctor` exited 0.
...
|---|---|---|---|
| Global | Current CLI resolves Cursor to `~/.agents/skills/topolyn`; Cursor officially scans that root. | A shell-capable Skill becomes available in every project; update timing is user-wide. | Default for an individual who wants Topolyn everywhere. Say exactly what scope it grants. |
| Project | Installs `.agents/skills/topolyn` plus `skills-lock.json` in the current repository. | The copied Skill is a substantial repository diff if committed, and can become stale relative to upstream. | Offer as the team/reproducible option, not as a silent default. |
...
- “Install Topolyn for Cursor with `--agent cursor`; use `.agents/skills/topolyn` for a project or a Cursor-supported global Skill root for user scope.”
- “The installed Topolyn package passes `doctor`, validation, and checked HTML delivery without `npm install`, provided Node.js 18+ is available.”
- “Topolyn uses the same portable `SKILL.md` and renderer contract across Cursor, Claude, Codex, and opencode.”
...
- “The global CLI command installs to `~/.cursor/skills/topolyn`.” The current implementation resolves Cursor as universal and may use `~/.agents/skills/topolyn`.
- “Cursor's Remote Rule UI imports Topolyn correctly from the bare repository URL.” Cursor documents the UI, but not nested Skill selection; Topolyn's actual Skill lives under `topolyn/`.
...
3. Reuse the same `topolyn/SKILL.md`; add no Cursor fork or Cursor-specific diagram behavior.
