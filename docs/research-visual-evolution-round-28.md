...
Topolyn already compiles authored `guidedViews`, renders the active chapter's
...
[`Topolyn.guidedViews`](../topolyn/assets/template.html) parses the existing
...
direct live-SVG serialization as Topolyn's export implementation. Topolyn's
...
selection. Topolyn already has deliberate Play/Pause ownership and should not
expand it.
...
ambient rotation, or full-page layout. The Topolyn rail must work by explicit
...
or a current-viewport export option in this slice. The current Topolyn
...
3. Do not change `Topolyn.exporter`, export cleanup, canonicality receipts,
...
boundaries keeps the result attractive, stable and recognizably Topolyn.
