...
chapter, Topolyn should show which stable node IDs would **stay in focus**, **enter
...
No inspected primary source ships this exact Topolyn interaction as a package.
...
[`Topolyn.guidedViews`](../topolyn/assets/template.html#L5424-L6252):
...
is inspected. **Adapt:** Topolyn needs one preview generation over stable SVG IDs,
...
would duplicate rendering and hide the precise semantic delta Topolyn can provide.
...
matters. **Adapt:** derive membership from independent Topolyn `focus` arrays; do
...
10. Focus, hover, and Escape do not write `activeIndex`, `aria-current`,
    `aria-pressed`, URL/hash/history, `Topolyn.focus`, Story Trail data,
...
Keep this inside the existing template-level `Topolyn.guidedViews` module so all
