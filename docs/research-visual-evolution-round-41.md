...
Build **Settled Flow**: an ordinary trace-enabled Topolyn artifact should run
...
Topolyn already has Story Trail, Story Beats, Story Follow Camera, Story
...
`topolyn/assets/template.html` currently gives trace edges and nodes:
...
source topology.
**Adapt:** Topolyn can reuse its existing CSS markers and six-second WebM
...
**Adapt:** Topolyn's Story features already own deliberate step navigation, so
...
model.
**Adapt:** treat Topolyn animation state as viewer-only; authored classes and
...
legible settled topology, with source geometry and meaning preserved. Topolyn
