...
Topolyn can already restore a named chapter, one focused node, one authored route,
...
[current shared viewer source](../topolyn/assets/template.html)).
...
resolved stop inside that projection. On load, Topolyn rebuilds the current node,
...
[viewer URL writers and readers](../topolyn/assets/template.html)):
...
([focus writer](../topolyn/assets/template.html),
[guided-view writer](../topolyn/assets/template.html),
[route writer](../topolyn/assets/template.html),
[Lens writer](../topolyn/assets/template.html)).
...
[current Story Beat implementation](../topolyn/assets/template.html)).
...
([Semantic Camera and export implementation](../topolyn/assets/template.html)).
...
**Adapt:** Topolyn's exact Story moment needs one narrower coordinate inside that
...
**Adapt:** use a stable node ID instead of a numeric fragment index. Topolyn node
...
coordinates into an Topolyn link. In React Flow those values are the editable
flow; in Topolyn the JSON IR and compiled SVG are canonical, and runtime state
...
breakpoint, embed size, browser zoom, or Presentation chrome. Topolyn can instead
derive an appropriate camera from the same semantic chapter nodes on the current
device. This is an Topolyn design inference based on React Flow's explicitly
pixel/zoom viewport model and Topolyn's current semantic camera implementation
([React Flow viewport definition](https://reactflow.dev/learn/concepts/terms-and-definitions#viewport),
[Topolyn camera source](../topolyn/assets/template.html)).
...
**Adapt:** Topolyn's view is already the board and its existing stable node ID is
...
**Adapt:** Topolyn can go one useful step further and document the story beat as a
...
| Tool | Stable semantic address | Exact incremental position | Raw viewport snapshot | Embed/autoplay meaning | Topolyn judgment |
|---|---|---|---|---|---|
...
| Topolyn now | `focus`, `route`, `lens`, `view` | Beat is in memory only | Derived camera, no URL snapshot | `play=1` is bounded chapter scope | Add `view + beat`, preserve everything else |
