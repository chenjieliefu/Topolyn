...
Topolyn already has optional trace animation, one-hop Intent Trace, directed
...
- [`template.html`](../topolyn/assets/template.html) already clones authored edge
...
Topolyn can borrow this invariant directly with far less machinery.
...
Topolyn's self-contained HTML viewer can apply the preference directly and should
...
removal decide when motion exists. Topolyn does not need G6's renderer, extension
...
For Topolyn's smaller fixed SVGs, the simpler equivalent is stronger: create at
...
The safer Topolyn grammar is a short one-shot directional pulse, with the static
...
12. Focused contract tests cover exact-edge ownership, direction, finite timing,
    ...
    Topolyn suite and Proof Lab remain green.
...
and zero new runtime architecture. That is enough movement to make Topolyn feel
