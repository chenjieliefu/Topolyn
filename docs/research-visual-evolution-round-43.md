...
copy Fireworks' complete showcase budget into Topolyn. Instead, it should move
...
This is deliberately a correctness gate, not an auto-router. Topolyn keeps the
...
- `topolyn/renderers/shared/geometry.mjs`: `segmentIntersectsRect` already
  provides the shared segment/rectangle primitive;
- `topolyn/renderers/workflow/render-workflow.mjs`: the only renderer that
  currently expands unrelated node boxes by 2px and rejects a crossing;
- `topolyn/renderers/architecture/render-architecture.mjs`,
...
- `topolyn/scripts/check-render-output.mjs`: performs a separate, post-render
...
`topolyn/test/layout-rules.test.mjs` has a focused negative fixture for a
workflow edge crossing a non-endpoint node, and
`topolyn/test/geometry.test.mjs` directly tests the segment/rectangle
...
**Adapt:** begin with the one invariant Topolyn can infer exactly from its
...
quality profile, not an unconditional retrofit over Topolyn's existing
...
classes.
**Adapt:** Topolyn should hard-gate peer node boxes and explicitly exempt
...
**Adapt:** retain Topolyn's current 2px workflow clearance in the first shared
gate so the rollout fixes inconsistency without silently tightening every
existing layout.
**Skip:** claiming a global zero-crossing guarantee when Topolyn is not running
...
message.
**Adapt:** Topolyn's receipt should point directly to `via`, `route`,
...
Topolyn currently has no schema concept for a junction, shared corridor,
...
**Topolyn never publishes a relationship through an unrelated semantic node,
...
exempt because crossing them can be semantically necessary and Topolyn has no
...
| Adapt | Reuse Topolyn's exact authored route points and the existing 2px workflow clearance. |
...
the right improvement is the small, strict statement Topolyn can prove today:
