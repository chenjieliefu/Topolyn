# Topolyn visual evolution research — round 2

Research date: 2026-07-19 (Asia/Shanghai)

## Executive decision

Topolyn should absorb **viewer intelligence**, **stable semantic identity**, and
...
associated with graph canvases without turning Topolyn into one.
...
checks keep it inside Topolyn's read-only viewer boundary.
...
- Motion is deliberately constrained by semantic roles and scene contracts,
  but its optional GIF path is a substantially heavier stack than Topolyn's
...
- a scenario suitability guide that tells users which Topolyn renderer and
...
- drag, resize, connect, lasso, whiteboard, collaborative editing, and a React
  runtime. Those solve canvas-authoring problems outside Topolyn's boundary.
...
- Its default visual renderer uses the browser Canvas API. That enables large,
  rich graphs, but Canvas does not itself give Topolyn the semantic DOM and
...
- Its extension surface includes minimaps, fisheye, tooltips, history, timebars,
  ...
  but also evidence that the full engine is much broader than Topolyn needs
...
- embedding G6 or its plugin ecosystem. Topolyn can implement the small,
...
- the conceptual split between an overview, alternate scenario, and ordered
  walkthrough; use it to shape Topolyn recipes and examples.
...
- Mermaid is embeddable through ESM/CDN and offers a smaller “tiny” build, but
  the full package has a broad runtime dependency graph. Topolyn should copy
...
- The JavaScript distribution supports Web Workers and ships a browser-ready
  ...
  Topolyn's current renderer-specific geometry
...
| Capability | Best evidence | Topolyn interpretation |
|---|---|---|
...
> Topolyn compiles typed technical intent into a deterministic, validated,
