...
- Topolyn source: `codex/cursor-onboarding`, based on `e7e22a1`

## Installation and discovery

The acceptance workspace was a new temporary directory. Topolyn was installed with the reproducible project command:
...
`skills list --agent cursor --json` reported one project-scoped `Cursor` installation at `.agents/skills/topolyn`. A new Cursor Agent session then independently reported that it had discovered:

```text
.agents/skills/topolyn/SKILL.md
```
...
Cursor was asked to use Topolyn to create a six-component local event-processing Architecture diagram containing:
...
An independent Topolyn `validate --json` and `check --json` rerun matched the nine passing checks, zero composition findings, byte count, and SHA-256 receipt.
...
- This record does not benchmark other Cursor models, claim automatic updates, or replace Topolyn's deterministic and perceptual delivery gates.
