...
numbers, while Topolyn does not yet serialize a route role rich enough to tell
...
Topolyn's current schemas; retain the three route budgets as metrics until
route-role calibration exists.
**Skip:** copying Fireworks' showcase numbers as immediate Topolyn failures or
...
**Adapt:** point to Topolyn's existing `via`, channel, side, and placement
...
- Border runs fail both Topolyn profiles; the Fireworks route-budget numbers
...
**A relationship may enter or leave a container, but Topolyn never lets the
...
because current Topolyn relationships target semantic nodes, not containers.
...
- Browser console logs contain no Topolyn runtime error.
