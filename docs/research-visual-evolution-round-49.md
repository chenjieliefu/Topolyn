...
- [`relationshipTokenKind()` and `relationshipTokenGeometry()`](../topolyn/assets/template.html)
...
- [`storyStep()` and `pulseStoryStep()`](../topolyn/assets/template.html) build
  ordered Story Beats and animate the generic Story Trail flow;
- [`storyMotionAllowed()`](../topolyn/assets/template.html) rejects every embed,
  even when the URL explicitly requested the existing `?embed=1&play=1` share
  contract;
- [`followStoryStep()`](../topolyn/assets/template.html), by contrast, correctly
...
- [shared Story Beat implementation](../topolyn/assets/template.html)
...
**Adapt:** Topolyn already has stronger stable `data-edge-key` / optional
...
**Adapt:** one carrier per selected Story Beat, with Topolyn's existing cleanup
and motion governor.
