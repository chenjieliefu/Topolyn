...
The reason to do this now is measurable. Topolyn has accumulated a powerful
...
diagram (`topolyn/assets/template.html`, current lines 3642–3679). Its first
...
bounded solution (`topolyn/assets/template.html`, current lines 7327–7342):
...
`2.75rem` controls (`topolyn/assets/template.html`, current lines 2737–2755).
...
**Adapt:** compact Topolyn's inactive story surface while retaining its stronger
...
**Adapt:** Topolyn should preserve Play and every named chapter before
activation, then reveal its richer director after activation.
**Skip:** hiding all navigation, requiring fullscreen, or reducing Topolyn to
only Previous/Next.
...
**Adapt:** use Topolyn's existing semantic state, not a new host configuration,
...
Topolyn does not need another diagram feature to look richer. It needs its
...
composition contract exposes a real gap in Topolyn's uneven cross-renderer
