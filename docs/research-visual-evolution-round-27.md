...
Topolyn already has two category surfaces:
...
> the existing Semantic Lens, and can that feel alive without turning Topolyn
...
[`Topolyn.semanticLens`](../topolyn/assets/template.html) currently:
...
- [`render-architecture.mjs`](../topolyn/renderers/architecture/render-architecture.mjs)
  ...
  bridge.
- [`render-workflow.mjs`](../topolyn/renderers/workflow/render-workflow.mjs)
...
- [`render-lifecycle.mjs`](../topolyn/renderers/lifecycle/render-lifecycle.mjs)
...
- [`render-sequence.mjs`](../topolyn/renderers/sequence/render-sequence.mjs)
  explains message/edge variants such as request and return. It is not a node
  category legend and must remain static.
- [`render-dataflow.mjs`](../topolyn/renderers/dataflow/render-dataflow.mjs)
...
of the live SVG for Topolyn's export path. Topolyn already has a stronger
canonical-clone boundary and should keep runtime decorations out of export.

This source is also a useful negative test: a legend can look like Topolyn's
...
The exact count badge is an Topolyn inference from the same category-to-ID
...
Topolyn must add keyboard, focus, touch, semantic state, and target-size behavior
...
This distinction maps cleanly to Topolyn:
...
For Topolyn's SVG-hosted legend rows, this means:
...
state. Topolyn should borrow it, with stricter limits:
...
- G6's exact category-to-element mapping, implemented through Topolyn's existing
...
- Edge-semantic legend lenses for Sequence and Data Flow, but only after Topolyn
...
3. Every visible count equals the corresponding value returned by
   `Topolyn.semanticLens.kinds()` after deduplication by `data-node-id`.
