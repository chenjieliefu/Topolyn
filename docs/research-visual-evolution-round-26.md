...
Topolyn should add one **finite, source-to-target Directional Flow Pulse** to the
...
- [`template.html`](../topolyn/assets/template.html) already resolves each
...
**Topolyn inference:** borrow the one bright head, exact path direction, immutable
...
**Topolyn inference:** Relationship Preview entry is the owning lifecycle stage.
...
**Topolyn inference:** keep the original edge as the hit target and semantic
...
background assumption. Topolyn already has a better hit target—the named
...
**Topolyn inference:** a CSS-only one-shot is smaller than a queued animation
...
An Topolyn relationship row is already a button. The HTML Standard forbids
