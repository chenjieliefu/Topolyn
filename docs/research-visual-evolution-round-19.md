...
Topolyn has strong committed exploration after a reader clicks a node: Semantic
...
Can Topolyn make the diagram feel immediately explorable without adding another
...
`clickNode`. The event payload includes the stable node ID. Topolyn should keep
...
and notes that labels or details can be delayed until tap/mouseover. Topolyn
...
triggered it. Topolyn already has the equivalent stable identity in
