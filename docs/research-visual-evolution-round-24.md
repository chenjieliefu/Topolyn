...
an interactive, counted **Semantic Lens** over the semantics Topolyn already
compiled—not a generic database-style filter builder.

## Current Topolyn evidence

- [`focusNodeAttrs()`](../topolyn/renderers/shared/cli.mjs) already emits a stable
...
- The viewer already owns temporary focus, route, story, camera, URL, print, and
  export state in [`template.html`](../topolyn/assets/template.html). Lens belongs
...
Topolyn should borrow that interaction grammar, not G6's plugin/canvas stack or
...
Topolyn already has the simpler SVG equivalent: set viewer-only match/context
...
set visible, and offer an obvious reset. Topolyn does not need nested property
...
Topolyn is a fixed-geometry technical-diagram compiler. A semantic lens should
...
It is excessive for Topolyn. The compiled diagram is the authoritative view;
...
authoring burden. It turns Topolyn's existing colors and node kinds from passive
