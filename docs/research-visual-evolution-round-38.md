...
look?” but not always “what happened here?” Topolyn already owns the facts needed
...
Topolyn has already solved the surrounding navigation layers:
...
**Adapt:** Topolyn should express semantic specificity as exact, readable beat
...
project, not general facts Topolyn can safely infer from its five typed IRs.
...
workspace navigation model. Topolyn's full static truth and local framing are
...
**Adapt now:** derive a concise explanation only from Topolyn's existing edge
...
**Adapt:** retain Topolyn's current pause-on-focus behavior and Live/Still owner.
...
| Per-diagram packet/train/ECG motion signatures | High spectacle | Low chrome, very high runtime surface | Needs explicit semantics Topolyn does not currently own | **Adapt later** |
...
feature has violated Topolyn's trust boundary. Fail closed to relation class,
