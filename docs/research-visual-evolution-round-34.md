...
Topolyn already exposes authored relationships in every typed renderer and can
...
([relationship renderer contract](../topolyn/renderers/shared/cli.mjs),
[current Relationship Lens](../topolyn/assets/template.html),
[current relationship tests](../topolyn/test/relationship-lens.test.mjs)).
...
orientation interaction. Topolyn already has node focus, one-hop intent,
...
> Should an Topolyn reader be able to interact with an authored relationship
...
of 2026-07-20. Product conclusions and proposed Topolyn behavior are explicitly
marked as recommendations or inferences rather than vendor claims.

## Current Topolyn baseline
...
([shared renderer helper](../topolyn/renderers/shared/cli.mjs)). The current
...
([relationship renderer tests](../topolyn/test/relationship-lens.test.mjs)).
...
[shared node accessibility helper](../topolyn/renderers/shared/cli.mjs)).
...
[current relationship geometry helper](../topolyn/assets/template.html)).
...
row visible
([Relationship Lens implementation](../topolyn/assets/template.html),
[Relationship Lens tests](../topolyn/test/relationship-lens.test.mjs)).

The gap is consequently not “invent an edge inspector.” It is “let the line open
the inspector state Topolyn already has.” Reusing the existing panel is important
...
embed variants
([shared viewer chrome](../topolyn/assets/template.html)).
...
([preview and pulse implementation](../topolyn/assets/template.html),
[export-clean regression tests](../topolyn/test/relationship-lens.test.mjs),
[pulse regression tests](../topolyn/test/relationship-pulse.test.mjs)).
...
channel
([Motion Governor implementation](../topolyn/assets/template.html),
[Motion Governor tests](../topolyn/test/motion-governor.test.mjs)).
...
([focus, route, Lens, and Guided Story hash readers](../topolyn/assets/template.html),
...
([shared renderer helper](../topolyn/renderers/shared/cli.mjs),
[typed renderer examples](../topolyn/examples)). Therefore Round 34 should keep
...
**Adapt:** Topolyn is a viewer, so activation means “inspect/pin this authored
...
contract is clearer for Topolyn.
...
**Adapt:** Topolyn already has SVG and can create genuine focusable DOM targets,
...
**Adapt:** Topolyn needs one pinned relationship, not yFiles' editor command
...
radius. This is an Topolyn implementation recommendation based on the W3C and
...
Topolyn should borrow the focus-management technique, not claim that diagram
...
**Borrow later:** if Topolyn adds relationship links, use an author-controlled
...
| Should pin create new UI? | Topolyn already has an exact Relationship Lens row and is chrome-dense. | No. Reuse and contextualize the existing Lens. |
...
| Is current identity sufficient for a deep link? | Graphviz requires author-controlled unique IDs; Topolyn's key is a source-order number. | No. Keep the pin local in Round 34 and do not emit a relation URL. |
...
- do not add a schema field, migration, server, storage, dependency, or editor
  API; and
- expose at most a small read-only `Topolyn.relationship` receipt/API for testing
...
works with mouse, touch, and keyboard, and composes with Topolyn's existing
