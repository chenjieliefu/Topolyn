...
Topolyn already has the major viewer primitives that modern graph tools use to
...
([Guided Story implementation](../topolyn/assets/template.html),
[Semantic Camera implementation](../topolyn/assets/template.html)).
...
first-glance hierarchy and reading comprehension—and what should Topolyn add
after its existing Radar, Camera, Reading Depth, Guided Story, Route Journey,
Motion Governor, and presentation features?

This note uses:

1. the current Topolyn worktree and generated Proof Lab artifacts;
...
4. explicit product inferences where a source supplies a primitive rather than
   an Topolyn-specific recommendation.
...
## Current Topolyn baseline: do not rebuild solved layers
...
([Semantic Radar implementation](../topolyn/assets/template.html),
[Semantic Radar tests](../topolyn/test/semantic-radar.test.mjs)).
...
([shared viewer CSS and camera](../topolyn/assets/template.html)).
...
([Reading Depth implementation](../topolyn/assets/template.html),
...
position means “default opening.” Topolyn should not infer cold-open intent from
...
([Story Beat tests](../topolyn/test/story-beat-navigator.test.mjs),
...
([camera implementation](../topolyn/assets/template.html),
[Route Journey tests](../topolyn/test/route-journey.test.mjs)).
...
Topolyn's existing camera. It should not store a React Flow viewport or make the
...
**Adapt:** Topolyn should not queue camera moves ahead of the reader. The Story
...
**Adapt:** Topolyn already has camera transaction IDs and settlement receipts.
...
**Adapt:** Topolyn's authored order is already `meta.views[].focus`; its Story
...
**Adapt:** Topolyn should preserve one SVG and express past/current/pending by
...
> Play or select a Story Beat, and Topolyn follows that exact authored moment
...
Call the already-public `Topolyn.view.reveal(ids, options)` from a small
...
- one shared Story-to-Camera request path in
  [`topolyn/assets/template.html`](../topolyn/assets/template.html);
- one story-owned caller of the existing public `Topolyn.view.reveal` method,
...
**Adapt** them into a Story Follow Camera over Topolyn's existing stable IDs,
...
observed legibility failure at the exact point Topolyn's narrative becomes most
