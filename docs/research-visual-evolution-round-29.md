...
Topolyn now has enough good motion primitives. Its next visual-quality problem is
...
- the existing Topolyn boundary: one deterministic self-contained HTML artifact,
...
[header and preset styling](../topolyn/assets/template.html#L519-L632),
[Radar styling](../topolyn/assets/template.html#L705-L752), and
[motion rules](../topolyn/assets/template.html#L2635-L2936).
...
- [ambient trace pause selectors](../topolyn/assets/template.html#L2933-L2936);
- [manual Story start](../topolyn/assets/template.html#L5467-L5483);
- [chapter activation and Semantic Camera reveal](../topolyn/assets/template.html#L5536-L5555).
...
See the current [reduced-motion block](../topolyn/assets/template.html#L2998-L3047)
...
may throttle them, but Topolyn has not expressed an application invariant.
...
### The existing `Topolyn.motion` name is already taken

`Topolyn.motion` is the public WebM recording surface with `canRecord()` and
`recordWebm()`. A governor must not replace or silently change this API. Use a
distinct owner such as `Topolyn.motionGovernor` and keep explicit motion export
separate from live-viewer preference.

See [the current recording implementation](../topolyn/assets/template.html#L3920-L4025)
and [animation contract tests](../topolyn/test/animation.test.mjs).
...
WAI Technique G186 is unusually direct for Topolyn: the control must be
...
It is Level AAA, but it matches Topolyn's stated quality target and existing
...
Topolyn's interactive artifact can offer reader control that a GIF cannot.
...
second persistent camera model. Topolyn's current 480ms bounded fit is enough.
...
**Skip now:** adding this extra movement before Topolyn can guarantee that
...
Keep the existing `Topolyn.motion.canRecord()` and
`Topolyn.motion.recordWebm()` export surface untouched.
...
8. `Topolyn.motionGovernor` exists without replacing or renaming
   `Topolyn.motion` WebM methods.
...
47. Explicit WebM recording remains available for trace artifacts through the
    existing `Topolyn.motion` API and contains no Governor UI/state.
