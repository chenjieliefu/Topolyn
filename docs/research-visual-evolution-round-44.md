...
lets Topolyn tighten polished examples without rejecting legitimate engineering
topologies.
...
`topolyn/renderers/shared/layout-report.mjs` serializes component and connection
...
of trusting coordinate equality; use Topolyn's real corpus to calibrate warning
budgets.
...
**Adapt:** only border-*collinearity* is universally wrong in Topolyn today;
...
- Keep shared-source/target channels measurable and non-blocking until Topolyn
...
**Topolyn tells an author whether a diagram is safe, whether it is polished,
...
| `composition/stacked-arrowhead` | multiple business arrowheads at the same endpoint coordinate | warning | warning | Fireworks rejects it, but Topolyn lacks port-offset controls today |
...
After explicit `port`, `junction`, and route-role semantics exist, Topolyn can
...
Round 44 is successful when Topolyn can truthfully say:
