...
Topolyn's story system can now explain the present unusually well. Story Follow
...
([viewer implementation](../topolyn/assets/template.html)).
...
node
([Story Follow contract](../topolyn/test/story-follow-camera.test.mjs)).
...
motion layer would compete with surfaces Topolyn already has. Story Horizon
...
**Adapt:** Topolyn should create expectation with one static immediate-next
...
**Adapt:** unlike a strict reveal, Topolyn should retain its stable topology and
...
**Adapt:** reuse Topolyn's existing `forward`, `reverse`, `multiple`, and
...
and composition semantics in Topolyn's source schema.
...
motion, and preserves Topolyn's strongest product boundary—one stable,
