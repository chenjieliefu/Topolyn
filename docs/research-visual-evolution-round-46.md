...
Topolyn's desired quality bar, or an aggregate score that cannot name a route.
...
to routing quality.
**Adapt:** compute Topolyn's `cross-container` class from typed structural
...
**Adapt:** use discrete explainable Topolyn classes instead of exposing router
...
**Adapt:** make ordinary Topolyn sequence messages an exact renderer invariant
...
**Adapt:** reuse Topolyn's existing small `role` vocabulary instead of adding a
...
**Topolyn keeps primary routes direct, gives necessary detours an explicit
...
The 8px terminal threshold is an Topolyn adaptation below the current 13px
...
- Desktop/mobile have no horizontal overflow or clipped receipt text; focus
  targets remain at least 44px; console contains no Topolyn runtime errors.
