...
Topolyn already has stronger semantic exploration, guided stories, export, and
...
What can Topolyn learn from `fireworks-tech-graph` without copying its breadth
...
3. The same reference contains a large product/brand catalogue. That breadth is
   ...
   matching responsibilities that Topolyn does not need for its core promise.
...
## Decision: Topolyn Semantic Sigils
...
## Why this adaptation fits Topolyn
...
- Twelve generic selectable skins: Topolyn keeps three coherent presets.
...
- Style-specific looping signatures for every scene: Topolyn already uses
