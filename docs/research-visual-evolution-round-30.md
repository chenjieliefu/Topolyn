...
Topolyn already has the right substrate: one canonical SVG, stable semantic node
...
3.2 seconds ([template](../topolyn/assets/template.html#L5784-L5890)). Semantic
...
([camera](../topolyn/assets/template.html#L6083-L6252)). The Governor already has
replaceable token ownership and stale-release protection
([governor](../topolyn/assets/template.html#L4373-L4582)). SVG export clones the
canonical graph and strips viewer transforms and overlays
([serialization](../topolyn/assets/template.html#L3709-L3830)).
...
| Adapt | Topolyn needs no cloned views or heuristic matching: both chapters reference the same persistent `data-node-id` elements. |
...
6. `Topolyn.view.reveal()` should return a transaction `{ id, state, finished,
