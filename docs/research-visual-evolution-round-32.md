...
Topolyn already turns each authored `meta.views[].focus` list into a visible Story
...
## Why this slice is real in current Topolyn

The implementation substrate already exists in
[`Topolyn.guidedViews`](../topolyn/assets/template.html):
...
request one finite signal over one exact Topolyn path, while the static beat
...
**Adapt:** Topolyn already has an authored stable-ID node sequence, a fixed SVG,
...
state, and previous/next semantics. **Adapt:** Topolyn's authored `focus` order is
...
recomputation, and any claim that prior Topolyn beats mutate the graph.
...
14. Expose a read-only deterministic receipt such as
    `Topolyn.guidedViews.beat()` returning `{index, total, nodeId, relation,
