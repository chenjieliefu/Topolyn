...
Build **First-fold Proof Aperture**: make a meaningful slice of Topolyn's
...
Topolyn already has the richer product: three live proof choices, five typed
...
because it is visible proof, not because Topolyn must copy twelve styles or a
GIF renderer. Adding another interaction to an already dense viewer would be
imitating surface breadth. Making Topolyn's own stronger artifact visible is
...
[`topolyn/test/landing.test.mjs`](../topolyn/test/landing.test.mjs) verifies that
...
`docs/assets/topolyn-live-proof.gif`, backed by a receipt and focused tests.
...
**Adapt:** expose Topolyn's existing real HTML proof rather than copying a
...
**Adapt:** Topolyn already has a richer interactive artifact, so reveal it
...
preview.
**Adapt:** keep Topolyn's current three proof tabs and full artifact link.
...
**Adapt:** retain Topolyn's read-only, generated artifact and keyboard proof
tabs.
...
| Adapt | Reuse the three checked Topolyn artifacts and existing tab semantics. |
...
Topolyn does not need another capability to become more convincing in this
...
and let the canvas own attention—while preserving Topolyn's narrower, stronger
