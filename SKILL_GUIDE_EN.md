...
Topolyn is an agent skill for Raven, Cursor, Claude Code, Codex CLI, and OpenCode. Give it a system description or repository; get an interactive, shareable technical map.

- **Open and present** — five diagram types, four presets, dark/light themes, brand marks, and finite motion
- **Review changes** — compare validated snapshots as Before / Delta / After with exact change facts
- **Explore grounded relationships** — search, inspect verified source, trace authored reach and routes, compare roles, and play guided stories
- **Share one checked file** — typed JSON and deterministic checks produce HTML, PNG, SVG, WebM, and 1200×630 cards
...
> **Current development version:** `v2.16.0-dev.0`. Core features are implemented; stable-release acceptance is in progress. See [Changelog](CHANGELOG.md#unreleased).
...
Then ask your agent: `Use Topolyn to map this repository's runtime architecture.`

## ❤️ Sponsors

<table>
  <tr><td align="center" width="240"><a href="https://apinebula.ai/ref/wywnaATT"><img src="docs/assets/sponsors/apinebula-topolyn.jpg" alt="APINEBULA" width="200" /></a><br/><strong><a href="https://apinebula.ai/ref/wywnaATT">APINEBULA</a></strong></td><td>APINEBULA sponsors Topolyn with one API for Claude, GPT, Gemini, and more. <a href="https://apinebula.ai/ref/wywnaATT">Register through Topolyn</a> and use <strong><code>Topolyn</code></strong> for <strong>10% off</strong>.</td></tr>
  <tr><td align="center" width="240"><a href="https://github.com/EverMind-AI/Raven"><img src="docs/assets/sponsors/evermind-topolyn-raven.png" alt="Topolyn × Raven" width="200" /></a><br/><strong><a href="https://github.com/EverMind-AI">EverMind</a> · <a href="https://github.com/EverMind-AI/Raven">Raven</a></strong></td><td>EverMind sponsors Topolyn and builds memory infrastructure for agents. Its <a href="https://github.com/EverMind-AI/Raven"><strong>Raven</strong></a> harness supports Topolyn as a Skill for verified, interactive system maps.</td></tr>
</table>

> Want to sponsor Topolyn? [Contact us by email.](mailto:2801884530@qq.com)

## See Topolyn in action

These are generated Topolyn artifacts, not product mockups. Click a frame to open its live, shareable state.

<p align="center">
  <a href="https://tt-a1i.github.io/archify/gallery.html"><img src="docs/assets/topolyn-live-proof.gif" alt="Three verified Topolyn artifacts moving through Signal Flow, Blueprint, and Classic presets" width="960"/></a>
  <br/>
...
|---|---|---|
| [![Agent workflow playing one authored chapter](docs/assets/topolyn-demo-story.png)](https://tt-a1i.github.io/archify/gallery/artifacts/agent-tool-call.workflow.html?theme=dark&present=1&play=1#view=happy-path) | [![Cache-miss sequence showing the Web App to Postgres route](docs/assets/topolyn-demo-route.png)](https://tt-a1i.github.io/archify/gallery/artifacts/cache-miss.sequence.html?theme=dark&present=1#route=web~db) | [![Production architecture comparing backend and database roles](docs/assets/topolyn-demo-lens.png)](https://tt-a1i.github.io/archify/gallery/artifacts/production-deployment.architecture.html?theme=dark&present=1#lens=backend~database) |
...
| Dark | Light |
|---|---|
| ![Dark theme](docs/assets/topolyn-dark.png) | ![Light theme](docs/assets/topolyn-light.png) |

The Export menu copies PNG to the clipboard and downloads static or motion formats:

![Export menu](docs/assets/topolyn-menu.png)
...
![Route Share Card showing the exact Users to API Server path with the full architecture retained as context](docs/assets/topolyn-route-share-card.png)
...
```bash
npx skills use chenjieliefu/Topolyn@topolyn --agent codex
```

[DSH community opt-in](integrations/deepseek-harness/README.md): `dsh plugin --profile web add @tt-a1i/archify-dsh@0.1.0`

The [agent switcher](https://tt-a1i.github.io/archify/start.html?agent=cursor&type=architecture) covers `cursor`, `codex`, `claude-code`, and `opencode`. For Raven's manual ZIP install, extract [`topolyn.zip`](topolyn.zip) into `~/.raven/workspace/skills`; it yields `~/.raven/workspace/skills/topolyn`. Raven is not a switcher target.

### 2. Ask for one bounded view

```text
Analyze this repository, then use Topolyn to create a high-level runtime architecture diagram.
...
```text
Use Topolyn to draw this login flow: Browser -> Web App -> API -> JWT validation ->
Redis session lookup -> PostgreSQL fallback. Keep the cache-miss path secondary.
```

### 3. Refine in chat

Continue with focused requests such as `add Redis`, `move auth to the left`, or `highlight the rollback path`. Topolyn keeps the typed source available for targeted iteration.
...
`node topolyn/bin/topolyn.mjs compare architecture base.json head.json architecture-delta.html --json`
...
```bash
node topolyn/bin/topolyn.mjs guide "Show an API request with Redis cache miss"
node topolyn/bin/topolyn.mjs guide "Map Kafka topics, consumer groups, replay, and DLQ" --json
```

Workflow keeps the happy path clear across lanes:

![Workflow example](docs/assets/topolyn-workflow.png)

Sequence explains one interaction over time:

![Sequence example](docs/assets/topolyn-sequence.png)

Data Flow makes movement and sensitivity boundaries explicit:

![Data Flow example](docs/assets/topolyn-dataflow.png)

Lifecycle separates progress, waits, retries, and terminal outcomes:

![Lifecycle example](docs/assets/topolyn-lifecycle.png)

Architecture examples: [`web-app`](examples/web-app.html) · [`Topolyn pipeline`](examples/topolyn-repo.html) · [`grid placement`](examples/topolyn-repo-grid.html) · [`desktop agent`](examples/maka-architecture.html)
...
Topolyn is not a general-purpose drawing editor or a Mermaid theme. It turns technical intent into a communication artifact.
...
node bin/topolyn.mjs guide "Show CI/CD checks, approval, deploy, and rollback"
node bin/topolyn.mjs validate workflow examples/agent-tool-call.workflow.json --quality showcase --json
node bin/topolyn.mjs preview workflow examples/agent-tool-call.workflow.json /tmp/workflow.html --quality showcase
node bin/topolyn.mjs deliver workflow examples/agent-tool-call.workflow.json /tmp/workflow.html --quality showcase --open --json
```
...
The complete generation and viewer contract lives in [`topolyn/SKILL.md`](topolyn/SKILL.md).

## Installation options

| Surface | Install location or method | Capability |
|---|---|---|
| **Raven** | Manual ZIP into `~/.raven/workspace/skills` → `~/.raven/workspace/skills/topolyn` | Full renderer + validation workflow |
...
| **Claude.ai** | Upload `topolyn.zip` under Settings → Capabilities → Skills | Depends on Node.js access in the sandbox |
| **Project Knowledge** | Upload `topolyn.zip` to the project | Prompt-driven architecture fallback |
**DeepSeek Harness:** Community integration, not an official DeepSeek product; developer-preview `@deepseek-ai/dsh@0.1.0-rc.6`, Node `^22.19.0 || >=24.0.0`. Install: `dsh plugin --profile web add @tt-a1i/archify-dsh@0.1.0`; invoke: `Use the Topolyn skill to map this repository's runtime architecture.`; remove: `dsh plugin --profile web remove @tt-a1i/archify-dsh`. No telemetry. Shell files need exact workspace paths, not Web Produced Files. [Details](integrations/deepseek-harness/README.md).

## Documentation and scope

- [Schema reference](topolyn/schemas/README.md) · [Skill](topolyn/SKILL.md) · [Examples](topolyn/examples/) · [Agent cookbook](docs/authoring-cookbook.md)
